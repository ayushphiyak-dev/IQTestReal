...
export function CommandMenu() {
  ...
  return <dialog open className="command-backdrop" aria-label="Quick navigation"><div className="command-dialog"><button className="command-close" onClick={() => setOpen(false)} aria-label="Close command menu">×</button><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search IQTestReal" aria-label="Search navigation"/><div>{filtered.map(({ label, href, icon: Icon }) => <button key={href} onClick={() => { setOpen(false); setQuery(''); router.push(href); }}><Icon size={17}/><span>{label}</span><kbd>↵</kbd></button>)}{filtered.length === 0 && <p>No matching destination.</p>}</div><footer><span>Navigate IQTestReal</span><span>ESC to close</span></footer></div></dialog>;
}
