...
import { BarChart3, BookOpen, BrainCircuit, CircleHelp, FileText, Search } from 'lucide-react';
...
const actions = [
  ...
  { label: 'Read IQ articles', href: '/articles', icon: FileText },
  { label: 'Frequently asked questions', href: '/faq', icon: CircleHelp },
  { label: 'About IQTestReal', href: '/about', icon: BrainCircuit },
...
export function CommandMenu() {
  ...
  if (!open) return <button type="button" className="command-hint" onClick={() => setOpen(true)} aria-label="Search IQTestReal (Ctrl + K or Command + K)" aria-keyshortcuts="Control+k Meta+k" title="Search IQTestReal (Ctrl + K or ⌘ + K)"><Search size={15}/> Ctrl + K</button>;
