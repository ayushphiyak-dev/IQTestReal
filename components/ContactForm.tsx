...
export function ContactForm() {
  ...
  async function submit(event: React.SubmitEvent<HTMLFormElement>) {
    ...
    const formData = {
      name: (form.elements.namedItem('nameInput') as HTMLInputElement).value,
      email: (form.elements.namedItem('emailInput') as HTMLInputElement).value,
      subject: (form.elements.namedItem('subjectInput') as HTMLInputElement)
        .value,
    ...
    try {
      ...
      if (!result.ok || !payload.ok)
        throw new Error(payload.message || 'Contact service unavailable');
      setResponse(payload.message || 'Message sent successfully!');
      setSuccess(true);
      form.reset();
    } catch (error) {
