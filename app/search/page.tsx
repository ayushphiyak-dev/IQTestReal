...
export default function SearchPage() {
  ...
  return <main id="main-content"><header className="page-header shell"><span className="eyebrow">Search / IQTestReal</span><h1>Find a useful page.</h1><p>Search the test, guides, articles, policies, and contact information.</p><div className="site-search"><Search size={19}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search IQTestReal" aria-label="Search IQTestReal" /></div></header><section className="search-results shell">{results.map((item) => <Link className="article-card" href={item.href} key={item.href}><span className="eyebrow">IQTestReal page</span><h2>{item.title}</h2><p>{item.description}</p><span className="text-link">Open page →</span></Link>)}{results.length === 0 && <p className="search-empty">No matching page. Try “test”, “score”, “privacy”, or “articles”.</p>}</section></main>;
}
